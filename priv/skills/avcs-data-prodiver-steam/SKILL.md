...
2. 调用脚本 `scripts/fetch_steam.py`，至少提供 `--game`。
3. 读取 JSON 返回。
4. 若 `status == success`：
...
```bash
cd <项目根目录>
python priv/skills/avcs-data-prodiver-steam/scripts/fetch_steam.py \
  --out-dir <输出目录> \
  --game <游戏名> \
  [--lang en-us|zh-cn|... ] \
  [--cc US] \
  [--cover-key header_image|capsule_image|capsule_imagev5|any|last]
```

- `--game`：必填，游戏名称或关键词。
- `--lang`：可选，Steam 接口语言，默认 `en-us`。
- `--cc`：可选，国家/地区码，默认 `US`。
- `--cover-key`：可选，优先下载封面类型，默认 `header_image`。
...
- 不支持强制从用户本地文件系统直接读取图片；下载封面仅限 `work/` 输出目录，交付请后续移入 `output/`。
