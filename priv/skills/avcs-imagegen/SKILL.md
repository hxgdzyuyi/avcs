...
- 不要创建或调用一次性图片生成脚本。
- 不要把 Agent 生成或加工结果保存到 `work/`；Avcs 项目内的生成、编辑和导出结果应进入 `output/`，除非用户明确指定其它项目内路径。
...
7. 检查结果：主体、风格、构图、文字准确性、保留项、避免项和 Avcs 用途是否满足。
8. 如需迭代，只做一个明确、局部的修正，再重新检查。
9. 预览用途可以直接展示图片；项目资产必须保存到当前项目内。
10. 保存项目资产时，优先放入 `<project>/output/`；如果用户指定项目内路径，则按用户路径保存。
11. 不要覆盖已有资产，除非用户明确要求替换；否则使用 `name-v2.png`、`name-edited.png`、`name-variant-01.png` 这类文件名。
12. 不要让项目引用的图片只停留在 `$CODEX_HOME/generated_images/...` 或其它默认生成目录。
13. 最终回复要说明：使用的是 built-in `image_gen`、最终保存路径、最终提示词或提示词集合。
...
2. 将选中的生成源图保存到当前项目 `output/`，文件名建议加 `-keyed`，例如 `badge-keyed.png`。
...
python <avcs-imagegen-skill-dir>/scripts/remove_chroma_key.py \
  --input <project>/output/<name>-keyed.png \
...
6. 最终项目引用透明文件，不引用 `-keyed` 源图；源图可保留在 `output/` 作为调试产物，也可按用户要求清理。
