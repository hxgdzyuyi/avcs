...
Avcs 是 local-first 的 AI Visual Content Studio。MVP 形态是由 Phoenix 提供统一入口的本地 Web 应用，用本地项目文件夹保存素材、输出和项目 SQLite，通过 Codex app-server 调用 Agent 能力。该本地 Web 应用可以被 Tauri 打包为 macOS Apple Silicon 桌面 app；Tauri 负责启动本地 Phoenix release、托盘菜单、deep link、文件关联和 updater 壳，并在系统默认浏览器中打开 Avcs，不改变 Avcs 的业务边界。
...
如果用户在 composer 中选择了 Data Provider，例如 `avcs-data-prodiver-apod`，本轮 `message:send` 会携带 provider 元信息。后端校验并持久化该配置，Agent runner 先指示 Codex 按 provider skill 获取来源数据，再使用 Avcs 图片生成链路产出图片。
...
在 Board 预览 dialog 中，用户可以对当前图片临时涂抹 mask 区域并发送编辑描述。由于 Codex built-in `image_gen` 未暴露正式 mask 参数，Avcs 将原图与导出的黑白 mask 图一起作为视觉引用传给 Agent，并附加“白色区域编辑、黑色区域尽量保持不变”的指令；该能力不等同于 Image API 的正式 mask edit。
...
11. Data Provider：React 只通过 WebSocket 发送已选择的 provider 元信息；provider skill 执行和本地脚本调用由后端 Agent runner 交给 Codex app-server 完成，React 不直接读取本地 skill 文件或执行脚本。
12. 图片 mask 编辑：预览 dialog 可使用局部 raster drawing layer 导出视觉参考 mask；Agent 仍通过 Codex app-server 接收原图、mask 图和文本指令，不直接接入独立图片编辑 API。
13. 桌面发布：GitHub Actions 中 `.app` 打包产物会经过 `codesign`（默认 adhoc）后再上传，不包含 Apple notarization。
14. 画板：普通 HTML DOM 元素自由布局，不使用 Canvas/WebGL/SVG 作为主渲染层。
15. 应用通信：React 与 Elixir 的状态同步主要使用 WebSocket；React 与 Elixir 的本地文件操作使用 Phoenix HTTP API；Elixir 与 Codex app-server 使用 stdio。
...
2. `ui.locale`：界面语言，默认 `en`，允许 `en` 和 `zh-hans`；该偏好只写入全局 SQLite，不写入项目 SQLite。
