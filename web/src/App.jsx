...
import { createBlankProject, deleteAsset, importAsset, openProject, readAssetPath, revealAsset, scanAssets, uploadAsset } from "./api.js";
...
export default function App() {
  ...
  const [composerSettings, setComposerSettings] = useState(defaultComposerSettings());
  ...
  useEffect(() => {
    ...
    async function applyRoute() {
      ...
      try {
        ...
        if (hasThread && (threadId !== currentThreadIdRef.current || draftThreadProjectIdRef.current === projectId)) {
          await handleSelectThread(threadId, projectId, { syncRoute: false });
          return;
        }
  ...
  const refreshAll = useCallback(
    async (threadId = currentThreadId, activeProject = project) => {
      ...
      setThreadsByProjectId((current) => ({ ...current, [activeProject.id]: threadItems }));
      setAssets(assetData.items || []);
      setBoardItems(boardData.items || []);
      setSelectedBoardIds((current) => current.filter((id) => (boardData.items || []).some((item) => item.id === id)));

      const keepDraft = draftThreadProjectIdRef.current === activeProject.id && !threadId;
      ...
      if (selectedThreadId) {
        const itemData = await channel.push("thread:items:list", { thread_id: selectedThreadId });
  ...
  const loadProjectThreads = useCallback(
    async (projectId) => {
      ...
      const data = await channel.push("threads:list", { project_id: projectId });
    ...
    const client = createAvcsChannel((event, payload) => {
      if (event === "project:updated") {
        ...
        if (payload.project?.id) {
          setExpandedProjectIds((current) => (current.includes(payload.project.id) ? current : [payload.project.id, ...current]));
        } else {
          clearWorkspaceState();
        }
      }
      ...
      if (event === "threads:updated") {
        if (payload.project_id) {
          setThreadsByProjectId((current) => ({ ...current, [payload.project_id]: payload.items || [] }));
        }
        if (!payload.project_id || payload.project_id === projectIdRef.current) {
          setThreads(payload.items || []);

          const hasCurrentThreadId = Object.prototype.hasOwnProperty.call(payload, "current_thread_id");
          if (hasCurrentThreadId && draftThreadProjectIdRef.current !== payload.project_id) {
            const nextThreadId = validThreadId(payload.items || [], payload.current_thread_id);
            currentThreadIdRef.current = nextThreadId;
            setCurrentThreadId(nextThreadId);
            setProject((current) => (current?.id === payload.project_id ? { ...current, current_thread_id: nextThreadId } : current));
            if (!nextThreadId) setItems([]);
          }
        }
      }
      if (event === "thread:items") {
        if (!belongsToCurrentThread(payload, currentThreadIdRef.current)) return;
        setItems(payload.items || []);
      }
      if (event === "item:created") {
        if (!belongsToCurrentThread(payload, currentThreadIdRef.current)) return;
        setItems((current) => upsertById(current, payload.item));
      }
      ...
      if (event === "assets:updated") {
        ...
        setReferences((current) => current.filter((id) => nextAssetIds.has(id)));
        setBoardItems((current) => current.filter((item) => nextAssetIds.has(item.asset_id)));
      }
      if (event === "board:items") {
        setBoardItems(payload.items || []);
        setSelectedBoardIds((current) => current.filter((id) => (payload.items || []).some((item) => item.id === id)));
      }
      if (event === "board:item:updated") {
        setBoardItems((current) => current.map((item) => (item.id === payload.item.id ? payload.item : item)));
      }
      if (event === "agent:run_started") {
        setRunningTurns((current) => ({
          ...current,
          [payload.thread_id]: { thread_id: payload.thread_id, turn_id: payload.turn_id, status: "running" },
        }));
        setStreamingByTurn((current) => ({
          ...current,
          [payload.turn_id]: { thread_id: payload.thread_id, turn_id: payload.turn_id, text: "" },
        }));
      }
      if (event === "assistant:delta") {
        if (!belongsToCurrentThread(payload, currentThreadIdRef.current)) return;
        setStreamingByTurn((current) => {
          const existing = current[payload.turn_id] || { thread_id: payload.thread_id, turn_id: payload.turn_id, text: "" };
          return {
            ...current,
            [payload.turn_id]: { ...existing, text: `${existing.text}${payload.delta || ""}` },
          };
        });
      }
      if (event === "tool:updated") {
        if (!belongsToCurrentThread(payload, currentThreadIdRef.current)) return;
        if (payload.item) setItems((current) => upsertById(current, payload.item));
      }
      if (event === "approval:requested") {
        setRunningTurns((current) => ({
          ...current,
          [payload.thread_id]: { thread_id: payload.thread_id, turn_id: payload.turn_id, status: "waiting_approval" },
        }));
        if (!belongsToCurrentThread(payload, currentThreadIdRef.current)) return;
        if (payload.item) setItems((current) => upsertById(current, payload.item));
      }
      ...
      if (event === "error" && belongsToCurrentThread(payload, currentThreadIdRef.current)) {
  ...
  useEffect(() => {
    ...
    const threadId = draftThreadProjectId === project.id ? null : project.current_thread_id;
  ...
  async function handleOpenProject(event) {
    ...
    try {
      ...
      setSelectedBoardIds([]);
      if (opened.id) {
        setExpandedProjectIds((current) => (current.includes(opened.id) ? current : [opened.id, ...current]));
      }
  ...
  async function handleCreateBlankProject() {
    ...
    try {
      ...
      setSelectedBoardIds([]);
      if (created.id) {
        setExpandedProjectIds((current) => (current.includes(created.id) ? current : [created.id, ...current]));
      }
      await refreshProjects();
  ...
  async function handleSelectProject(projectId, { syncRoute = true } = {}) {
    ...
    if (projectId === project?.id && draftThreadProjectId !== projectId) {
      if (syncRoute) syncProjectWorkspaceRoute(projectId, currentThreadId, { force: true });
      return;
    }
    setNotice("");
    setDraftProjectId(null);
    ...
    if (projectId === project?.id) {
      const selectedThreadId = await refreshAll(project.current_thread_id, project);
      if (syncRoute) syncProjectWorkspaceRoute(projectId, selectedThreadId, { force: true });
      return;
    }
    ...
    try {
      ...
      if (syncRoute) syncProjectWorkspaceRoute(selectedProject.id, selectedProject.current_thread_id, { force: true });
      ...
      setSelectedBoardIds([]);
      setExpandedProjectIds((current) => (current.includes(projectId) ? current : [projectId, ...current]));
      const selectedThreadId = await refreshAll(selectedProject.current_thread_id, selectedProject);
      if (syncRoute) syncProjectWorkspaceRoute(selectedProject.id, selectedThreadId, { force: true });
    } catch (error) {
      setNotice(error.message);
    }
  }
  ...
  async function handleArchiveProject(projectEntry) {
    ...
    if (!window.confirm(`归档项目“${projectEntry.name}”？项目文件夹不会被删除。`)) return;
    setNotice("");

    try {
      const data = await channel.push("project:archive", { id: projectEntry.id });
  ...
  async function handleDeleteProject(projectEntry) {
    if (!channel || !projectEntry?.id) return;
    if (!window.confirm(`从项目列表删除“${projectEntry.name}”？这只会删除 SQLite 中的项目引用，不会删除文件夹。`)) return;
    setNotice("");

    try {
      const data = await channel.push("project:delete", { id: projectEntry.id });
      applyProjectRemoval(projectEntry.id, data.project);
      setNotice("项目引用已删除");
    } catch (error) {
      setNotice(error.message);
  ...
  function applyProjectRemoval(projectId, activeProject) {
    ...
    setShowAllThreadProjectIds((current) => current.filter((id) => id !== projectId));
  ...
  async function handlePrepareNewThread(projectId = project?.id, { syncRoute = true } = {}) {
    if (!channel || !projectId) return;
    setNotice("");
    ...
    try {
      ...
      if (syncRoute) syncProjectWorkspaceRoute(projectId, null, { force: true });
    } catch (error) {
      setNotice(error.message);
    }
  }
  ...
  async function createThreadForDraft() {
    const thread = await channel.push("thread:create", { title: "Untitled thread" });
    setDraftProjectId(null);
    currentThreadIdRef.current = thread.id;
    setCurrentThreadId(thread.id);
    setProject((current) => (current ? { ...current, current_thread_id: thread.id } : current));
    setItems([]);
    setSelectedBoardIds([]);
  ...
  async function handleSelectThread(id, projectId = project?.id, { syncRoute = true } = {}) {
    if (!channel) return;
    let activeProject = project;
    ...
    setProject((current) => (current?.id === activeProject?.id ? { ...current, current_thread_id: id } : current));
    setPrompt("");
    setReferences([]);
    setSelectedBoardIds([]);
    setMobileView("thread");
    ...
    if (syncRoute) syncProjectWorkspaceRoute(activeProject?.id || projectId, id, { force: true });
  }
  ...
  async function handleDeleteThread(id) {
    ...
    const activeProject = project ? { ...project, current_thread_id: nextThreadId } : project;
    setProject(activeProject);
  ...
  async function handleSend() {
    if (!channel || !project || agentRunning) return;
    const text = prompt.trim();
    if (!text && references.length === 0) return;
    setNotice("");

    try {
      const thread = currentThreadId ? { id: currentThreadId } : await createThreadForDraft();
  ...
  async function handleApprovalRespond(item, decision) {
    ...
    try {
      ...
      if (data.item) setItems((current) => upsertById(current, data.item));
    } catch (error) {
      setNotice(error.message);
    }
  }
  ...
  async function handleRepairThread() {
    ...
    try {
      const data = await channel.push("thread:repair", { thread_id: repairThreadId });
      ...
      if (project?.id) setThreadsByProjectId((current) => ({ ...current, [project.id]: nextThreads }));
      setAssets(nextAssets);
      setBoardItems(nextBoardItems);
      setSelectedBoardIds((current) => current.filter((id) => nextBoardItems.some((item) => item.id === id)));
      setReferences((current) => current.filter((id) => nextAssets.some((asset) => asset.id === id)));

      const repair = data.repair || {};
      setNotice(`Thread repaired: ${repair.matched_turns || 0} turns, ${repair.synced_items || 0} items synced.`);
    } catch (error) {
      setNotice(error.message);
    } finally {
  ...
  async function handleImportPath(path) {
    if (!path) return;
    try {
  ...
  async function refreshAssetsAndBoard() {
    if (!channel) return;
    const [assetData, boardData] = await Promise.all([channel.push("assets:list"), channel.push("board:items:list")]);
    setAssets(assetData.items || []);
    setBoardItems(boardData.items || []);
    setSelectedBoardIds((current) => current.filter((id) => (boardData.items || []).some((item) => item.id === id)));
  }
  ...
  function addReference(assetId) {
    setReferences((current) => (current.includes(assetId) ? current : [...current, assetId]));
  }
  ...
  async function handleMove(id, x, y, commit) {
    setBoardItems((current) => current.map((item) => (item.id === id ? { ...item, x, y } : item)));
    if (commit && channel) {
      try {
        await channel.push("board:item:move", { id, x, y });
      } catch (error) {
        setNotice(error.message);
        await refreshAssetsAndBoard();
      }
  ...
  async function handleResize(id, displayWidth, displayHeight, commit) {
    setBoardItems((current) => current.map((item) => (item.id === id ? { ...item, display_width: displayWidth, display_height: displayHeight } : item)));
    if (commit && channel) {
      try {
        await channel.push("board:item:resize", { id, display_width: displayWidth, display_height: displayHeight });
      } catch (error) {
        setNotice(error.message);
        await refreshAssetsAndBoard();
      }
  ...
  async function handleDeleteSelectedBoardItem({ item, selectedCount } = {}) {
    ...
    if (!window.confirm(`删除图片“${fileName}”？这会通过 rm 删除单个文件，操作不可撤销。`)) return;
    ...
    try {
      ...
      setBoardItems((current) => current.filter((candidate) => candidate.id !== item.id && candidate.asset_id !== item.asset_id));
      setAssets((current) => current.filter((candidate) => candidate.id !== item.asset_id));
      await refreshAssetsAndBoard();
      setNotice("图片已删除");
    } catch (error) {
      setNotice(error.message);
  ...
  function syncProjectWorkspaceRoute(projectId, threadId, { force = false } = {}) {
  ...
  const currentThread = useMemo(() => (isDraftThread ? null : threads.find((thread) => thread.id === currentThreadId)), [
    isDraftThread,
    threads,
    currentThreadId,
  ]);
  ...
  useEffect(() => {
    if (!project?.id || tracingThreadId || isProjectRouteSyncRef.current) return;

    syncProjectWorkspaceRoute(project.id, isDraftThread ? null : currentThreadId);
  }, [project?.id, isDraftThread, currentThreadId, routePath]);

  useEffect(() => {
    setComposerSettings(settingsFromThread(currentThread));
  }, [
    currentThread?.id,
  ...
  const activeRun = !isDraftThread && currentThreadId ? runningTurns[currentThreadId] : null;
  const agentRunning = Boolean(activeRun);
  const threadRepairing = !isDraftThread && currentThreadId ? Boolean(repairingThreads[currentThreadId]) : false;
  const anyAgentRunning = Object.keys(runningTurns).length > 0;
  const runningThreadIds = Object.keys(runningTurns);
  const streamingText = activeRun ? streamingByTurn[activeRun.turn_id]?.text || "" : "";
    ...
    <>
      <nav className="mobile-tabs" aria-label="Workspace views">
        {[
          ...
          <button className={mobileView === value ? "active" : ""} type="button" key={value} onClick={() => setMobileView(value)}>
            {label}
          </button>
        ))}
      </nav>
      ...
      <div className="app-shell" data-mobile-view={mobileView}>
          ...
          <div className="workbench-body">
            <ChatPane
              ...
              onRemoveReference={(assetId) =>
                setReferences((current) => current.filter((id) => id !== assetId))
              }
            ...
            <BoardPane
              ...
              currentThreadId={isDraftThread ? null : currentThreadId}
        ...
        {project ? <AssetImportShortcut onImport={handleImportPath} disabled={!project} /> : null}
      </div>
    </>
  );
}

function AssetImportShortcut({ onImport, disabled }) {
  ...
  return (
    ...
    >
      <input value={path} disabled={disabled} onChange={(event) => setPath(event.target.value)} placeholder="/absolute/image/path" />
...
function upsertThread(threads, thread) {
  ...
  return threads.map((entry) => (entry.id === thread.id ? { ...entry, ...thread } : entry));
}
...
function parseProjectRoute(pathname) {
  const match = String(pathname || "").match(/^\/web\/projects\/([^/]+)(?:\/threads\/([^/]+))?\/?$/);
...
function defaultComposerSettings() {
  return {
    model: "",
    effort: "",
...
function settingsFromThread(thread) {
  if (!thread) return defaultComposerSettings();

  return {
    model: thread.default_model || "",
    effort: thread.default_effort || "",
    approval_policy: thread.default_approval_policy || "never",
    sandbox_mode: thread.default_sandbox_mode || "workspace-write",
  };
}
